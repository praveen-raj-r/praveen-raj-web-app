...
export default function NotFound() {
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-neutral-950">
      {/* Soft Gradient Glow */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_20%,#7a77ff33,transparent_70%)] blur-xl" />

      {/* Floating Orbs */}
      <div className="absolute w-72 h-72 bg-[#7a77ff22] rounded-full blur-3xl -top-20 -left-20 animate-pulse" />
      <div className="absolute w-72 h-72 bg-[#7a77ff22] rounded-full blur-3xl -bottom-20 -right-20 animate-pulse delay-300" />

      {/* Glass Card */}
      <div className="relative z-10 rounded-3xl px-8 py-14 md:px-16 md:py-20 flex flex-col items-center text-center">
        {/* Animated Icon */}
        <div className="opacity-90 animate-pulse">
          ...
          >
            <g filter="url(#filter0_dd_2882_16663)">
              <circle cx="132" cy="113.5" fill="#1a1a1a" r="32.5" shapeRendering="crispEdges" />
              <circle cx="132" cy="113.5" r="33" shapeRendering="crispEdges" stroke="#ffffff24" />
            </g>
            <path
        ...
        <Button
          onClick={() => router.back()}
          className="group cursor-pointer relative mb-10 px-8 py-3 text-base font-medium rounded-xl bg-[#0d0d10] text-white border border-[#2b2b33] shadow-[0_3px_10px_#00000072] transition-all duration-300 hover:bg-[#141418] hover:shadow-[0_4px_14px_#00000099] active:scale-95 active:shadow-[0_2px_6px_#00000066] overflow-hidden"
        >
          <span className="relative z-10 tracking-wide">Escape the Void</span>
          <span className="absolute inset-x-0 bottom-0 h-1/2 bg-[radial-gradient(ellipse_at_bottom,#7d78ff33,transparent_70%)] opacity-0 group-hover:opacity-80 transition-opacity duration-500" />
        </Button>
        ...
        <p className="text-neutral-300 text-lg md:text-xl mt-4 max-w-lg leading-relaxed">
          Looks like this page slipped through a crack in the universe. Let&apos;s
          get you back on track.
        </p>
      </div>

      <div className="absolute inset-0 z-0 opacity-[0.08] bg-[linear-gradient(90deg,#fff2_1px,transparent_1px),linear-gradient(#fff2_1px,transparent_1px)] bg-size-[60px_60px]" />
    </div>
  );
}
